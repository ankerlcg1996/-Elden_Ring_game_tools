...
public class CreateLoadoutViewModel : BaseViewModel
{
    ...
    {
        string name = _showInputDialog("输入新套装名称：", "");
    ...
    private void RenameLoadout()
    {
        ...
        string newName = _showInputDialog("输入套装新名称：", SelectedLoadout.Name);
    ...
    private void ExportLoadout()
    {
        ...
        try
        {
            ...
            MsgBox.Show($"已导出套装：{SelectedLoadout.Name}");
        }
        catch (Exception ex)
        {
            MsgBox.Show($"导出套装失败：{ex.Message}");
        }
    }
