...
constexpr const char* kEzStateDispatcherPattern = "48 8B DA 4C 8B F1 33 FF 89 7D";
...
bool EnsureEzStateDispatcher() {
    ...
    const uintptr_t pattern = FindPatternInText(kEzStateDispatcherPattern);
    if (pattern == 0) {
        return false;
    }

    g_EzStateDispatch = reinterpret_cast<EzStateTalkDispatchFn>(pattern - 0x47);
...
void InspectorTools::Tick(const Game::SingletonRegistry& singletons) {
    ...
    ERD_PROTECTED_STEP("InspectorTools.LoadNpcMenuCatalog", LoadNpcMenuCatalog());
    ...
    ERD_PROTECTED_STEP("InspectorTools.SyncNpcMenus", SyncNpcMenus(singletons));
...
void InspectorTools::RefreshSnapshot(const Game::SingletonRegistry& singletons) {
    ...
    next.npc_menus = snapshot_.npc_menus;
    next.npc_menu_error = snapshot_.npc_menu_error;
...
void InspectorTools::LoadNpcMenuCatalog() {
    const fs::path path = GetResourcePath(L"NpcMenus.zh.txt");
    if (path.empty() || !fs::exists(path)) {
        std::scoped_lock lock(snapshot_mutex_);
        snapshot_.npc_menus.clear();
        snapshot_.npc_menu_error = "未找到外置 NPC 菜单表: Resources/NpcMenus.zh.txt";
        return;
    }

    const auto write_time = fs::last_write_time(path);
    if (loaded_npc_menu_path_ == path && loaded_npc_menu_time_ == write_time) {
        return;
    }

    std::ifstream input(path);
    std::vector<NpcMenuEntrySnapshot> entries;
    std::string line;
    int next_id = 1;
    while (std::getline(input, line)) {
        ...
        NpcMenuEntrySnapshot entry{};
        entry.id = next_id++;
        entry.category = parts[0];
        entry.label = parts[1];
        entry.event_id = ParseIntSafe(parts[2], 0);
        entry.use_player_handle = (_stricmp(parts[3].c_str(), "player") == 0);
        entry.description = parts.size() >= 6 ? parts[5] : "";

        std::size_t start = 0;
        int parsed = 0;
        while (start <= parts[4].size() && parsed < static_cast<int>(entry.params.size())) {
            const std::size_t separator = parts[4].find(',', start);
            const std::string token =
                separator == std::string::npos
                    ? TrimCopy(parts[4].substr(start))
                    : TrimCopy(parts[4].substr(start, separator - start));
            if (!token.empty()) {
                entry.params[static_cast<std::size_t>(parsed++)] = static_cast<std::int64_t>(std::stoll(token, nullptr, 0));
            }
            if (separator == std::string::npos) {
                break;
            }
    ...
    {
        std::scoped_lock lock(snapshot_mutex_);
        snapshot_.npc_menus = std::move(entries);
        snapshot_.npc_menu_error.clear();
    }
    loaded_npc_menu_path_ = path;
    loaded_npc_menu_time_ = write_time;
}
...
void InspectorTools::SyncNpcMenus(const Game::SingletonRegistry& singletons) {
    const int request_id = Main::g_FeatureStatus.npc_menu_request_id.exchange(0);
    if (request_id <= 0) {
        return;
    }

    NpcMenuEntrySnapshot requested{};
    {
        std::scoped_lock lock(snapshot_mutex_);
        auto found = std::find_if(snapshot_.npc_menus.begin(), snapshot_.npc_menus.end(), [request_id](const NpcMenuEntrySnapshot& entry) {
            return entry.id == request_id;
        });
        if (found == snapshot_.npc_menus.end()) {
            Main::g_FeatureStatus.npc_menu_last_result = -1;
            Main::Logger::Instance().Error(("Npc menu id not found: " + std::to_string(request_id)).c_str());
            return;
        }
        requested = *found;
    }

    const std::uint64_t handle = requested.use_player_handle ? GetPlayerHandle(singletons) : static_cast<std::uint64_t>(-1);
    const bool success = ExecuteEzStateEvent(handle, requested.event_id, requested.params, requested.param_count);
    Main::g_FeatureStatus.npc_menu_last_result = success ? 1 : -1;
    if (success) {
        Main::Logger::Instance().Info(("Npc menu opened: " + requested.label).c_str());
    } else {
        Main::Logger::Instance().Error(("Npc menu failed: " + requested.label).c_str());
    }
}
