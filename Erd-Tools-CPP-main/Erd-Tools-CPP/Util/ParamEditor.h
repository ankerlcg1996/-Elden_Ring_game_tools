...
public:
    ...
    void AddNewParams();
    bool ReloadParam(std::vector<uint8_t>&& rebuiltParam);
...
private:
    ...
    static inline std::shared_ptr<std::vector<uint8_t>> _reloadedParamStorage;
};
...
void ParamEditor<T>::AddNewParams() {
    ...
    constexpr size_t kParamPrefixSize = 0x10;
    std::vector<uint8_t> newParamStorage(newParamSize + kParamPrefixSize, 0);
    *(uint32_t*)(newParamStorage.data()) = static_cast<uint32_t>(newParamSize);
    uint8_t* newParamRaw = newParamStorage.data() + kParamPrefixSize;

    //Copy over the data from the beginning of the old param to the start of the data offset.
    ParamHeader* newParamHeader = (ParamHeader*)newParamRaw;
    ...
    if (!ReloadParam(std::move(newParamStorage))) {
        throw std::runtime_error("Failed to reload param " + std::string(T::param_name) + " after rebuilding.\n");
    }
}

template <typename T>
bool ParamEditor<T>::ReloadParam(std::vector<uint8_t>&& rebuiltParam) {
    if (rebuiltParam.size() <= 0x10) {
        return false;
    }

    _reloadedParamStorage = std::make_shared<std::vector<uint8_t>>(std::move(rebuiltParam));
    auto* newParamHeader = reinterpret_cast<ParamHeader*>(_reloadedParamStorage->data() + 0x10);
    if (newParamHeader == nullptr) {
        return false;
    }

    _paramResCap->ParamInfo->Param = newParamHeader;
    _paramHeader = newParamHeader;
    _paramPointer = reinterpret_cast<uintptr_t>(newParamHeader);
    _paramTable = &_paramHeader->Table;
    return true;
}
...
ParamResCap* ParamEditor<T>::getParamResCap() {
    ...
    const int entryCount =
        static_cast<int>(sizeof(_soloParamRepository->repositoryEntries) / sizeof(_soloParamRepository->repositoryEntries[0]));
